...
- Opensource & Free: no monthly license fee for hosting providers (Windows Server license can up to $20/VM/month)
- Super-low resources consuming at production hosting: less CPU overhead, 20M-100M of RAM (if compare to GB of RAM in Windows Server requirements)
- Super-cheap hosting: can be hosted in $2/month VM & very easy to find GIANT low-end hosting providers (Vultr, Linode, DO)
- Highly customizable OS distribution for the need of deployment, instead of single OS for all purpose if compare to Windows Servers
- Huge community as well as huge toolset/technologies/components for server side management
